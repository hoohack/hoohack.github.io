...
![TCP三次握手](http://7u2eqw.com1.z0.glb.clouddn.com/tcp3shake.gif)
...
![TCP断开连接](http://7u2eqw.com1.z0.glb.clouddn.com/Tcp4shake.gif)
...
![TCP-status-classic](http://7u2eqw.com1.z0.glb.clouddn.com/TCPStatus-Classic.jpg)
...
客户端收到服务器的断开连接分节FIN（N）后，发送确认分节ACK给服务器，并进入TIME-WAIT状态，此时会等待足够的时间，大约是最长分节生命期的2倍（2MSL），确认服务器收到断开连接的确认分节，之后就会消失。
服务器收到客户端的断开连接的确认分节ACK（N+1）后，表示连接已经完全断开了，此时进入CLOSED状态

看完上面的一大段文字可能有点枯燥，甚至有点懵，来看看这张动画图：

![连接断开-TCP状态变化](http://7u2eqw.com1.z0.glb.clouddn.com/TcpStatus.gif)
